...
struct Env {
  ...
  Func *lookup_func(std::string_view name) {
    ...
    if (res != glob.end()) {
      if (Func *f = res->second.as_func()) { return f; }
    }
    ERR("no such func %s", CSTR(name));
  }

  Decl *lookup_decl(std::string_view name) {
    for (auto it = local_stk.rbegin(), end = local_stk.rend(); it < end; ++it) {
      auto res = it->find(name);
      if (res != it->end()) { return res->second; }
    }
    auto res = glob.find(name);
    if (res != glob.end()) {
      if (Decl *d = res->second.as_decl()) { return d; }
    }
    ERR("no such variable %s", CSTR(name));
  }

  void ck_func(Func *f) {
    cur_func = f;
    if (!glob.insert({f->name, Symbol::mk_func(f)}).second) {
      ERR("duplicate function %s", CSTR(f->name));
    }
    local_stk.emplace_back(); // 参数作用域就是第一层局部变量的作用域
    for (Decl &d : f->params) {
      ck_decl(d, false);
      if (!local_stk[0].insert({d.name, &d}).second) {
        ERR("duplicate param decl %s", CSTR(d.name));
      }
    }
    for (Stmt *s : f->body.stmts) { ck_stmt(s); }
    local_stk.pop_back();
  }
  ...
  void ck_decl(Decl &d, bool is_glob) {
    ...
    for (auto begin = d.dims.rbegin(), it = begin, end = d.dims.rend(); it < end; ++it) {
      ...
      if (e) { // 函数参数中dims[0]为nullptr
        eval(e);
        if (e->result < 0) { ERR("array dim < 0"); }
        if (it != begin) { e->result *= it[-1]->result; }
      }
    }
    if (d.has_init) {
      if (d.dims.empty() && d.init.val1) { // 形如int x = 1
        if (d.is_const) { eval(d.init.val1); }
  ...
  void ck_stmt(Stmt *s) {
    if (auto x = dyn_cast<Assign>(s)) {
      ...
      x->lhs_sym = d;
      if (d->is_const) { ERR("can't assign to const decl"); }
      for (Expr *e : x->dims) { if (!is_int(ck_expr(e))) { ERR("index operator expect int operand"); }}
      if (!(is_int(ck_expr(x->rhs)) && d->dims.size() == x->dims.size())) { ERR("can only assign int to int"); }
      ...
      for (Decl &d : x->decls) {
        ck_decl(d, false);
        if (!top.insert({d.name, &d}).second) { ERR("duplicate local decl %s", CSTR(d.name)); }
      }
    } else if (auto x = dyn_cast<Block>(s)) {
      local_stk.emplace_back();
      for (Stmt *s : x->stmts) { ck_stmt(s); }
      local_stk.pop_back();
    } else if (auto x = dyn_cast<If>(s)) {
      if (!is_int(ck_expr(x->cond))) { ERR("cond isn't int type"); }
      ck_stmt(x->on_true);
      if (x->on_false) { ck_stmt(x->on_false); }
      ...
      if (!loop_cnt) { ERR("break out of loop"); }
    } else if (auto x = dyn_cast<Continue>(s)) {
      if (!loop_cnt) { ERR("continue out of loop"); }
  ...
  void flatten_init(std::vector<InitList> &src, Expr **dims, Expr **dims_end, bool need_eval, std::vector<Expr *> &dst) {
    u32 elem_size = dims + 1 < dims_end ? dims[1]->result : 1, expect = dims[0]->result * elem_size;
    u32 cnt = 0, old_len = dst.size();
    for (InitList &l : src) {
      if (l.val1) {
        if (need_eval) { eval(l.val1); }
        dst.push_back(l.val1);
        if (++cnt == elem_size) { cnt = 0; }
    ...
    if (actual <= expect) {
      ...
      ERR("too many initializing values, expect %d, found %d", expect, actual);
    }
  }
  ...
  std::pair<Expr **, Expr **> ck_expr(Expr *e) {
    ...
    if (auto x = dyn_cast<Binary>(e)) {
      ...
      if (!is_int(l) && !is_int(r)) { ERR("binary operator expect int operands"); }
      return empty;
    } else if (auto x = dyn_cast<Unary>(e)) {
      auto r = ck_expr(x->rhs);
      if (!is_int(r)) { ERR("unary operator expect int operand"); }
      return empty;
    } else if (auto x = dyn_cast<Call>(e)) {
      Func *f = lookup_func(x->func);
      if (f->params.size() != x->args.size()) { ERR("function call argc mismatch"); }
      for (u32 i = 0; i < x->args.size(); ++i) {
        ...
        for (u32 j = 1, end = p.size(); ok && j < end; ++j) {
          if (a.first[j]->result != p[j]->result) { ok = false; }
        }
        if (!ok) { ERR("function call arg %d mismatch", i + 1); }
      }
      return f->is_int ? empty : none;
    } else if (auto x = dyn_cast<Index>(e)) {
      // 这里允许不完全解引用数组
      Decl *d = lookup_decl(x->name);
      if (x->dims.size() > d->dims.size()) { ERR("index operator expect array operand"); }
      for (Expr *e : x->dims) { if (!is_int(ck_expr(e))) { ERR("index operator expect int operand"); }}
  ...
  void eval(Expr *e) {
    if (auto x = dyn_cast<Binary>(e)) {
      ...
      switch (x->tag) {
        case Expr::Add: x->result = l + r; break;
        case Expr::Sub: x->result = l - r; break;
        case Expr::Mul: x->result = l * r; break;
        // 除0就随它去吧，反正我们对于错误都是直接退出的
        case Expr::Div: x->result = l / r; break;
        case Expr::Mod: x->result = l % r; break;
        case Expr::Lt: x->result = l < r; break;
        case Expr::Le: x->result = l <= r; break;
        case Expr::Ge: x->result = l >= r; break;
        case Expr::Gt: x->result = l > r; break;
        case Expr::Eq: x->result = l == r; break;
        case Expr::Ne: x->result = l != r; break;
        case Expr::And: x->result = l && r; break;
        case Expr::Or: x->result = l || r; break;
        default: UNREACHABLE();
      }
    } else if (auto x = dyn_cast<Unary>(e)) {
      eval(x->rhs);
      i32 r = x->rhs->result;
      switch (x->tag) {
        case Expr::Neg: x->result = -r; break;
        case Expr::Not: x->result = !r; break;
        default: UNREACHABLE();
      }
    } else if (auto x = dyn_cast<Call>(e)) {
      // 常量表达式中不能包含函数调用
      ...
      Decl *d = lookup_decl(x->name);
      if (!d->is_const) { ERR("non-constant variable %s used in constant expr", CSTR(x->name)); }
      // 常量表达式中必须完全解引用数组
      if (d->dims.size() != x->dims.size()) { ERR("index expression array dim mismatch"); }
      u32 off = 0;
      ...
      if (off >= d->flatten_init.size()) { ERR("constant index out of range"); }
...
void typeck(Program &p) {
  Env env;
  for (Func &f : Func::BUILTIN) { env.ck_func(&f); }
  for (auto &g : p.glob) {
    if (Func *f = std::get_if<0>(&g)) {
      ...
      if (!env.glob.insert({d->name, Symbol::mk_decl(d)}).second) {
        ERR("duplicate decl %s", CSTR(d->name));
      }
    }
  }
}